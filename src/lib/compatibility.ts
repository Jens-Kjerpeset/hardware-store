...
export function checkCompatibility(products: SelectedProduct[]): string[] {
  ...
  if (cpu && motherboard) {
    if (cpu.socket !== motherboard.socket) {
      warnings.push(`Incompatible Socket: CPU ${cpu.productName} uses ${cpu.socket}, but Motherboard ${motherboard.productName} uses ${motherboard.socket}.`);
    }
  }
  ...
  if (motherboard && ramList.length > 0) {
    for (const ram of ramList) {
      if (ram.memoryType !== motherboard.memoryType) {
         warnings.push(`Incompatible RAM: Motherboard ${motherboard.productName} uses ${motherboard.memoryType}, but ${ram.productName} is ${ram.memoryType}.`);
      }
    }
  }
  ...
  if (cooler && cooler.compatibleSockets && (motherboard || cpu)) {
    ...
    if (targetSocket && !cooler.compatibleSockets.includes(targetSocket)) {
      warnings.push(`Incompatible Cooler: ${cooler.productName} does not support the ${targetSocket} socket.`);
    }
  }
  ...
  if (psu && psu.wattage) {
    if (gpu && gpu.recommendedPsu) {
      if (psu.wattage < gpu.recommendedPsu) {
        warnings.push(`Insufficient Power: GPU ${gpu.productName} recommends ${gpu.recommendedPsu}W, but PSU ${psu.productName} only provides ${psu.wattage}W.`);
      }
    } else if (cpu && cpu.tdp) {
      // If no GPU is present, add a 100W buffer to the CPU TDP requirement
      const minRequired = cpu.tdp + 100;
      if (psu.wattage < minRequired) {
        warnings.push(`Insufficient Power: System requires at least ${minRequired}W, but PSU ${psu.productName} provides ${psu.wattage}W.`);
      }
    }
  }
  ...
  if (caseItem && caseItem.formFactor?.includes('ITX')) {
    warnings.push(`Notice: Small Form Factor (Mini-ITX) cases have strict limits on GPU length and often require SFX Power Supplies. Double check clearance before building.`);
  }
...
export function checkPotentialCompatibility(buildProducts: SelectedProduct[], potentialProduct: SelectedProduct): string[] {
  ...
  // Also ignore Notices
  const newWarnings = allWarnings.filter(w => !currentWarnings.includes(w) && !w.startsWith('Notice:'));
  return newWarnings;
}

export function getIncompatibleFilterOptions(buildProducts: SelectedProduct[], category: string): Record<string, string[]> {
  ...
  const ALL_FORM_FACTORS = ['E-ATX', 'ATX', 'Micro-ATX', 'mATX', 'Mini-ITX', 'ITX'];
  const ALL_CASE_MAINBOARDS = ['E-ATX', 'ATX', 'Micro-ATX', 'Mini-ITX'];

  if (category === 'CPU') {
    if (motherboard && motherboard.socket) {
      incompatible.sockets = ALL_SOCKETS.filter(s => s !== motherboard.socket);
    }
  }
  ...
  if (category === 'RAM') {
    if (motherboard && motherboard.memoryType) {
      incompatible.memoryTypes = ALL_MEMORY_TYPES.filter(m => m !== motherboard.memoryType);
    }
  }
  ...
  if (category === 'CPU Cooler') {
    const targetSocket = (motherboard && motherboard.socket) || (cpu && cpu.socket);
    if (targetSocket) {
