...
import { checkCompatibility, SelectedProduct } from '@/lib/compatibility';
import { useMemo } from 'react';
import { AlertTriangle, CheckCircle2, ChevronRight, X, Plus, Minus, Trash2, Info, ShoppingCart } from 'lucide-react';
import Link from 'next/link';
import { useRouter } from 'next/navigation';

const REQUIRED_BUILD_CATEGORIES = ['CPU', 'CPU Cooler', 'Motherboard', 'GPU', 'RAM', 'Primary Storage', 'Power Supply', 'Case', 'OS'];
...
export default function CatalogLayout({
  ...
  const { mode, buildSystem, removeBuildComponent, cart, removeFromCart, updateQuantity, addToCart, clearBuild, setMode } = useStore();
  ...
  const currentTotal = mode === 'build' 
    ? buildParts.reduce((sum, item) => sum + item.price, 0)
    : cart.reduce((sum, item) => sum + item.price * item.quantity, 0);

  const cartCount = mode === 'build' ? Object.keys(buildSystem).length : cart.reduce((sum, item) => sum + item.quantity, 0);
  ...
  const missingCategories = REQUIRED_BUILD_CATEGORIES.filter(cat => !buildSystem[cat]);
  ...
  return (
    <div className="flex flex-col md:flex-row gap-8 w-full animate-in fade-in duration-500">
      ...
      {(mode === 'build' || mode === 'loose') && (
        <aside className="w-full md:w-80 shrink-0 glass h-fit sticky top-24 border-dark-border shadow-2xl flex flex-col overflow-hidden bg-dark-bg/90">
            ...
            <div className="flex flex-col gap-4 p-4 max-h-[60vh] overflow-y-auto">
               ...
               <div className="space-y-3 mt-2">
                  ...
                  {currentItems.length === 0 ? (
                    ...
                    currentItems.map(item => (
                      <div key={item.id} className="flex flex-col p-3 bg-dark-surface/50 border border-dark-border group">
                         <div className="flex justify-between items-start mb-1">
                           <div className="text-[10px] text-gray-500 font-bold uppercase tracking-wider">{item.category.name}</div>
                           <button 
                             onClick={() => mode === 'build' ? removeBuildComponent(item.category.name) : removeFromCart(item.id)} 
                             className="text-gray-500 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity p-0.5"
                           >
                              <X className="w-3 h-3" />
                           </button>
                         </div>
                         <div className="text-sm font-medium text-gray-200 line-clamp-2 leading-snug mb-2">
                           {item.name}
                         </div>
                         <div className="flex justify-between items-end mt-auto pt-2">
                           {mode === 'loose' ? (
                             <div className="flex items-center gap-2 bg-dark-bg border border-dark-border p-0.5 rounded-sm">
                                <button onClick={() => updateQuantity(item.id, (item as any).quantity - 1)} className="w-5 h-5 flex items-center justify-center text-gray-400 hover:text-white bg-white/5 text-xs">-</button>
                                <span className="text-[10px] font-mono w-4 text-center">{(item as any).quantity}</span>
                                <button onClick={() => updateQuantity(item.id, (item as any).quantity + 1)} className="w-5 h-5 flex items-center justify-center text-gray-400 hover:text-white bg-white/5 text-xs">+</button>
                             </div>
                           ) : <div />}
                           <div className="font-mono text-xs text-amber-400 font-bold text-right">
                             ${(mode === 'loose' ? item.price * (item as any).quantity : item.price).toFixed(2)}
                           </div>
                         </div>
                      </div>
                    ))
             ...
             <div className="p-4 border-t border-dark-border bg-dark-surface/80">
               <div className="flex justify-between text-gray-300 mb-4 items-end">
                 ...
                 <span className="font-black text-white text-xl leading-none">${currentTotal.toFixed(2)}</span>
               </div>
