...
export default function CatalogLayout({
  ...
  const cartCount =
    mode === "build"
      ? Object.keys(buildSystem).length +
        buildStorage.reduce((sum, item) => sum + item.quantity, 0)
  ...
  const allMissing = missingCategories.map((cat) => ({
    label: cat,
    path: cat,
  }));
  if (!hasAtLeastOneSSD) {
    allMissing.push({ label: "SSD", path: "Storage" });
  }
        ...
        <aside className="w-full lg:w-80 shrink-0 glass h-fit lg:sticky top-24 border-dark-border shadow-2xl flex flex-col overflow-hidden bg-dark-bg/90">
          <div className="flex items-center justify-between p-4 border-b border-dark-border bg-dark-surface/50">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative flex items-center justify-center text-gray-300">
                ...
                {cartCount > 0 && (
                  <span className="absolute -top-2 -right-2 bg-brand-500 text-white text-[11px] font-bold px-1.5 py-0.5  shadow-lg">
                    {cartCount}
                  </span>
                )}
              </div>
          ...
          <div className="flex flex-col gap-4 p-4 max-h-[60vh] overflow-y-auto">
                ...
                {notices.length > 0 && (
                  <div className="mt-3 space-y-2">
                    ...
                    <ul className="text-xs text-blue-200/80 space-y-1 ml-6 list-disc">
                      {notices.map((w: string, i: number) => (
                        <li key={i}>{w.replace("Notice: ", "")}</li>
                      ))}
                    </ul>
                  </div>
                )}
                ...
                {missingCategories.length > 0 || !hasAtLeastOneSSD ? (
                  <div className="space-y-2 mt-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {allMissing.map((item) => (
                        <Link key={item.label} href={`/catalog/${item.path}`}>
                          <span className="px-2 py-1 bg-amber-500/10 text-amber-400 text-[11px] border border-amber-500/20 hover:bg-amber-500/20 hover:text-amber-300 transition-colors cursor-pointer block">
                            {item.label}
                          </span>
                        </Link>
                      ))}
                  ...
                  <div className="flex items-center gap-2 text-emerald-400 text-xs font-bold mt-2">
                    <CheckCircle2 className="w-5 h-5" /> Rig is fully complete
                    and compatible!
                  </div>
                ) : null}
              </div>
            ...
            <div className="space-y-3 mt-2">
              ...
              {currentItems.length === 0 ? (
                <div className="text-center text-gray-500 py-6 text-sm border border-dashed border-dark-border">
                ...
                currentItems.map((item) => {
                  ...
                  return (
                    <div
                      ...
                      onClick={() =>
                        router.push(
                          `/catalog/${encodeURIComponent(item.category.name)}`,
                        )
                      }
                    ...
                    >
                      <div className="flex justify-between items-start mb-1">
                        <div
                          className={`text-[11px] font-bold   flex items-center gap-1.5 ${isIncompatible ? "text-red-400" : "text-gray-500"}`}
                        >
                          {isIncompatible && (
                          ...
                          onClick={(e) => {
                            ...
                            if (mode === "build") {
                              if (item.category.name === "Storage")
                                removeBuildStorageComponent(item.id);
                      ...
                      <div className="flex justify-between items-end mt-auto pt-2">
                        {mode === "loose" ||
                        (mode === "build" &&
                          item.category.name === "Storage") ? (
                          <div className="flex items-center gap-2 bg-dark-bg border border-dark-border p-0.5 ">
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                if (mode === "loose")
                              ...
                              className="w-5 h-5 flex items-center justify-center text-gray-400 hover:text-white bg-white/5 text-xs"
                            >
                              -
                            </button>
          ...
          <div className="p-4 border-t border-dark-border bg-dark-surface/80">
            <div className="flex justify-between text-gray-300 mb-4 items-end">
              ...
              <span className="font-black text-white text-xl leading-none font-mono">
            ...
            <button
              onClick={() => {
                if (mode === "loose" || canCheckoutBuild)
            ...
            >
              {mode === "loose" || canCheckoutBuild
                ? "Checkout"
                : "Fix Missing Parts/Errors"}{" "}
