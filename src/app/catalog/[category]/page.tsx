...
import { Cpu, ShoppingCart, FilterX, ChevronDown, Check, ChevronLeft, HelpCircle, AlertTriangle } from 'lucide-react';
...
const CATEGORY_BRAND_MAP: Record<string, string[]> = {
  ...
  'Primary Storage': ['Samsung', 'Western Digital', 'Crucial', 'Seagate'],
  'Additional Storage': ['Samsung', 'Western Digital', 'Crucial', 'Seagate'],
...
const CPU_MIN_SPEED = 4.0;
const CPU_MAX_SPEED = 6.5;
const CPU_MIN_CORES = 6;
const CPU_MAX_CORES = 24;
const CPU_MAX_TDP = 200;
const CPU_SOCKETS = ['AM5', 'LGA1700', 'LGA1851'];

const GPU_MIN_VRAM = 4;
const GPU_MAX_VRAM = 24;
const GPU_MIN_PSU = 400;
const GPU_MAX_PSU = 1200;
const GPU_INTERFACES = ['PCIe 3.0', 'PCIe 4.0', 'PCIe 5.0'];
const GPU_CHIPSETS = ['NVIDIA', 'AMD', 'Intel'];

// Motherboard Filters
const MOTHERBOARD_FORM_FACTORS = ['ATX', 'mATX', 'ITX', 'E-ATX'];
const MOTHERBOARD_MEMORY_TYPES = ['DDR4', 'DDR5'];
const MOTHERBOARD_SOCKETS = ['AM4', 'AM5', 'LGA1700', 'LGA1851'];
const MOTHERBOARD_CHIPSETS = ['Z890', 'Z790', 'B760', 'B650E', 'B650', 'B550'];
const GPU_MAX_LENGTH = 450;

// RAM Filters
const RAM_MIN_SPEED = 2133;
const RAM_MAX_SPEED = 8400;
const RAM_MAX_CAS = 50;
const RAM_CAPACITIES = [8, 16, 32, 64, 96];
const RAM_MODULES = ['2x8GB', '2x16GB', '2x24GB', '2x32GB', '4x8GB', '4x16GB'];

// Shared Filter Constants
const STORAGE_CAPACITIES = [500, 1000, 2000, 4000, 8000, 14000, 20000];

// Primary Storage Filters
const STORAGE_INTERFACES = ['PCIe 3.0', 'PCIe 4.0', 'PCIe 5.0', 'SATA'];
const STORAGE_MIN_READ_SPEED = 500;
const STORAGE_MAX_READ_SPEED = 15000;

// Additional Storage Filters
const STORAGE_TYPES = ['HDD', 'SSD'];
const STORAGE_FORM_FACTORS = ['2.5"', '3.5"'];

// Power Supply Filters
const PSU_MIN_WATTAGE = 400;
const PSU_MAX_WATTAGE = 1600;
const PSU_EFFICIENCIES = ['80+ White', '80+ Bronze', '80+ Gold', '80+ Platinum', '80+ Titanium'];
const PSU_MODULARITIES = ['Full', 'Semi', 'Non-Modular'];
const PSU_FORM_FACTORS = ['ATX', 'SFX', 'SFX-L'];

// Case Filters
const CASE_FORM_FACTORS = ['Full Tower', 'Mid Tower', 'Mini-ITX'];
const CASE_MAX_MAINBOARDS = ['E-ATX', 'ATX', 'Micro-ATX', 'Mini-ITX'];
const CASE_COLORS = ['Black', 'White'];
const CASE_SIDE_PANELS = ['Tempered Glass', 'Mesh', 'Solid'];

// CPU Cooler Filters
const COOLER_TYPES = ['Air', 'Liquid'];
const COOLER_RADIATOR_SIZES = [120, 240, 280, 360, 420];
const COOLER_COLORS = ['Black', 'White', 'Brown/Silver'];
const MIN_COOLER_NOISE = 0;
const MAX_COOLER_NOISE = 50;
const MIN_COOLER_HEIGHT = 0;
const MAX_COOLER_HEIGHT = 200;
...
export default function CategoryGrid({ params }: { params: Promise<{ category: string }> }) {
  ...
  // Zustand Store
  const { mode, addToCart, setBuildComponent, buildSystem } = useStore();
  const [addedItem, setAddedItem] = useState<string | null>(null);

  const buildParts = Object.values(buildSystem);
  ...
  // Deep Filters (Primary Storage)  // Storage specific filters
  ...
  const fetchProducts = useCallback(async () => {
    ...
    if (decodedCategory === 'Primary Storage' || decodedCategory === 'Additional Storage') {
  ...
  return (
    <div className="flex flex-col gap-6 w-full animate-in fade-in duration-500">
      ...
      <Link href="/catalog" className="flex items-center gap-2 text-gray-400 hover:text-white transition-colors w-fit group text-sm font-medium">
         <ChevronLeft className="w-4 h-4 group-hover:-translate-x-1 transition-transform" /> Back to Categories
      </Link>

      <div className="flex flex-col md:flex-row gap-8 w-full">
        {/* Sidebar filters */}
        <aside className="w-full md:w-56 shrink-0 glass p-6 h-fit border-dark-border shadow-xl">
            ...
            <div className="pt-6 border-t border-dark-border mt-6">
              ...
              <div className="space-y-2">
                {CPU_SOCKETS.map((s) => {
                  ...
                  return (
                  <label key={s} className={`flex items-center gap-3 cursor-pointer group select-none p-1.5 -ml-1.5 transition-colors ${isFilterIncompatible ? 'border border-red-500/50 bg-red-950/30' : 'border border-transparent'}`}>
                    <input 
                      type="checkbox" 
                      checked={isChecked}
                      onChange={toggle}
            ...
            <div className="pt-6 border-t border-dark-border mt-6">
              ...
              <div className="space-y-2">
                {MOTHERBOARD_FORM_FACTORS.map((f) => {
                  ...
                  return (
                  <label key={f} className={`flex items-center gap-3 cursor-pointer group select-none p-1.5 -ml-1.5 transition-colors ${isFilterIncompatible ? 'border border-red-500/50 bg-red-950/30' : 'border border-transparent'}`}>
                    <input 
                      type="checkbox" 
                      checked={isChecked}
                      onChange={toggle}
              ...
              <div className="space-y-2">
                {MOTHERBOARD_MEMORY_TYPES.map((t) => {
                  ...
                  return (
                  <label key={t} className={`flex items-center gap-3 cursor-pointer group select-none p-1.5 -ml-1.5 transition-colors ${isFilterIncompatible ? 'border border-red-500/50 bg-red-950/30' : 'border border-transparent'}`}>
                    <input 
                      type="checkbox" 
                      checked={isChecked}
                      onChange={toggle}
          ...
          {/* Primary Storage Deep Filters */}
          {decodedCategory === 'Primary Storage' && (
            <div className="pt-6 border-t border-dark-border mt-6">
              ...
              <div className="flex items-center mb-3 mt-8">
                <h3 className="text-sm font-semibold text-brand-500 uppercase tracking-wider">Interface</h3>
                <FilterTooltip text="The connection standard used by the NVMe drive. PCIe 5.0 is the newest and fastest, but your motherboard must support it to get full speeds." />
              </div>
              <div className="space-y-2">
          ...
          {/* Additional Storage Deep Filters */}
          {decodedCategory === 'Additional Storage' && (
            <div className="pt-6 border-t border-dark-border mt-6">
              ...
              <div className="flex items-center mb-3">
                <h3 className="text-sm font-semibold text-brand-500 uppercase tracking-wider">Form Factor</h3>
                <FilterTooltip text="3.5-inch drives are standard desktop sizes (mostly HDDs). 2.5-inch drives are smaller (mostly SATA SSDs and laptop drives)." />
              </div>
              <div className="space-y-2 mb-8">
              ...
              <div className="flex items-center mb-3">
                <h3 className="text-sm font-semibold text-brand-500 uppercase tracking-wider">Capacity (GB)</h3>
                <FilterTooltip text="The total storage space available on the drive." />
              </div>
              <div className="space-y-2">
                {STORAGE_CAPACITIES.map((c) => {
                  const isChecked = storageCapacities.includes(c);
      ...
      <section className="flex-1 flex flex-col gap-6">
        ...
        {/* Toolbar */}
        <div className="relative flex flex-col sm:flex-row justify-between items-center sm:items-center gap-4 bg-dark-surface/50 p-4 border border-dark-border">
           <div className="relative w-full sm:w-64">
             <input 
                ...
                className="w-full bg-dark-bg/50 border border-dark-border text-white px-4 py-2 focus:outline-none focus:border-brand-500 transition-colors"
             />
           </div>

           <div className="hidden md:flex items-center text-center absolute left-1/2 -translate-x-1/2">
             <select 
               value={decodedCategory}
               onChange={(e) => router.push(`/catalog/${e.target.value}`)}
               className="text-xl font-bold tracking-normal text-white opacity-80 decoration-brand-500 decoration-2 underline-offset-4 bg-transparent appearance-none focus:outline-none cursor-pointer hover:opacity-100 transition-opacity pr-6 relative z-10"
             ...
             <ChevronDown className="absolute right-0 top-1/2 -translate-y-1/2 w-5 h-5 text-brand-500 pointer-events-none" />
           </div>
           ...
           <div className="relative w-full sm:w-auto">
             <select 
               ...
               className="w-full sm:w-auto appearance-none bg-dark-bg/50 border border-dark-border text-white px-4 py-2 pr-10 focus:outline-none focus:border-brand-500 transition-colors cursor-pointer"
             >
                ...
                {(decodedCategory === 'Primary Storage' || decodedCategory === 'Additional Storage') && (
                  <>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {products.map(product => {
               ...
               return (
                <div key={product.id} className={`group flex flex-col glass transition-all duration-300 relative ${isIncompatible ? 'border-red-500/50 hover:shadow-[0_0_30px_-5px_rgba(239,68,68,0.2)] hover:z-50' : 'hover:border-brand-500/50 hover:shadow-[0_0_30px_-5px_rgba(59,130,246,0.2)] hover:z-50'}`}>
                  ...
                  <div className="p-5 flex flex-col flex-1">
                    ...
                    <h3 className="text-lg font-bold text-gray-100 leading-tight mb-2 group-hover:text-white transition-colors">
                      {product.name}
                    </h3>
                    ...
                    <div className="flex items-center justify-between mt-auto pt-4 border-t border-dark-border">
                      ...
                      <button 
                        ...
                        className={`h-10 px-4 font-bold text-white flex items-center justify-center shadow-lg transition-all hover:scale-105 active:scale-95 gap-2 ${mode === 'build' ? 'bg-amber-600 hover:bg-amber-500 hover:shadow-amber-500/30' : 'bg-brand-600 hover:bg-brand-500 hover:shadow-brand-500/30'}`}
                      >
                         {mode === 'build' ? (
                           <span>Select</span>
