...
export default function HardwareStoreGame() {
  ...
  useEffect(() => {
    ...
    const townImg = new Image();
    townImg.src = '/sprites/Game end.png';

    // NPC Definitions Map
    const loadImg = (src: string) => { const img = new Image(); img.src = src; return img; };
    type NPCType = { id: string, walk?: HTMLImageElement, idle?: HTMLImageElement, scaleModifier?: number, fpsModifier?: number, flipDefault?: boolean, isPickup?: boolean };
    ...
    let nextPickupTime = 223.0 - 5.0; // Starting 5 seconds in
    let unspawnedPickups = [
    ...
    const render = (time: number) => {
      ...
      for (let i = entities.length - 1; i >= 0; i--) {
        ...
        if (!ent.hit) {
          ...
          if (!isCinematicOutro && pDrawX < eDrawX + eDrawW && pDrawX + pDrawW > eDrawX && 
              ...
              // 2. Narrow Phase Pixel-Perfect Collision via composite intersecting rect
                  ...
                  if (iw >= 1 && ih >= 1) {
                      ...
                      if (ent.npcType.isPickup) {
                          offCtx.drawImage(ent.sprite, 0, 0, ent.sprite.naturalWidth, ent.sprite.naturalHeight, -eDrawW / 2, -eDrawH / 2, eDrawW, eDrawH);
                      } else {
         ...
         if (ent.frameTimer >= finalFPS) {
             ...
             if (ent.npcType.id === 'Pirate') {
                 ...
                 if (!isCinematicOutro && ent.currentFrame === 1 && ent.x + ent.width < canvas.width) {
         ...
         {!isPlaying && (
           <div className="absolute inset-0 z-20 flex flex-col items-center justify-center rounded bg-zinc-950/20">
              <img 
                ...
                className="w-[600px] [@media(max-height:500px)]:w-[350px] max-w-[80%] object-contain mb-12 [@media(max-height:500px)]:mb-4 drop-shadow-[0_20px_20px_rgba(0,0,0,0.8)] hover:scale-105 transition-transform duration-[2000ms] ease-in-out"
              />
              <button 
                onClick={() => setIsPlaying(true)}
                className="bg-brand text-white font-black text-2xl [@media(max-height:500px)]:text-xl px-12 py-4 [@media(max-height:500px)]:px-8 [@media(max-height:500px)]:py-3 rounded-lg shadow-[0_0_40px_rgba(234,88,12,0.8)] hover:bg-brand-hover hover:scale-105 active:scale-95 transition-all outline-none"
              >
                PLAY
              </button>
           </div>
...
function VirtualJoystick({ onMove }: { onMove: (x: number, y: number) => void }) {
    ...
    return (
        <div 
           className="absolute bottom-6 left-6 z-[160] w-32 h-32 bg-white/10 border-2 border-white/20 rounded-full flex items-center justify-center md:hidden backdrop-blur-sm touch-none"
           ref={baseRef}
        ...
        >
           <div ref={stickRef} className="w-12 h-12 bg-white/40 border border-white/50 rounded-full shadow-[0_0_15px_rgba(255,255,255,0.4)] pointer-events-none transition-transform duration-75" />
        </div>
    );
}
