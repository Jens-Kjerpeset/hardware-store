      ...
      <div className="bg-surface border border-border p-6  space-y-6">
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
           <div>
             ...
             <input disabled={isPending} {...register('code')} placeholder="SUMMER50" className="w-full bg-background border border-border  px-3 py-2 text-white font-mono uppercase focus:outline-none focus:border-brand" />
      ...
      <div className="bg-background border border-border p-6  space-y-6">
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
           <div>
             ...
             <select disabled={isPending} {...register('discountType')} className="w-full bg-surface border border-border  px-3 py-2 text-white focus:outline-none focus:border-brand font-medium">
           ...
           {currentType === 'PERCENTAGE' ? (
             <div>
               ...
               <input disabled={isPending} type="number" step="1" {...register('discountPercent')} className="w-full bg-surface border border-brand  px-3 py-2 text-white focus:outline-none" />
             ...
             <div>
               ...
               <input disabled={isPending} type="number" step="0.01" {...register('discountAmount')} className="w-full bg-surface border border-brand  px-3 py-2 text-white focus:outline-none" />
           ...
           <div className="md:col-span-2">
             ...
             <input disabled={isPending} type="number" step="0.01" {...register('minOrderValue')} className="w-full bg-surface border border-border  px-3 py-2 text-white focus:outline-none focus:border-brand" />
           </div>
           ...
           <div className="md:col-span-2 space-y-2 pt-2 border-t border-border">
             ...
             <div className="grid grid-cols-2 md:grid-cols-3 gap-2 bg-surface p-4  border border-border overflow-y-auto max-h-40">
                {categories.map(c => (
      ...
      <div className="bg-surface border border-border p-6  space-y-6">
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
           <div>
             ...
             <input disabled={isPending} type="number" placeholder="Leave empty for unlimited" {...register('maxUses')} className="w-full bg-background border border-border  px-3 py-2 text-white focus:outline-none focus:border-brand" />
           </div>
           
           <div>
             <label className="block text-sm font-semibold text-zinc-400 mb-1">Expiration Date</label>
             <input disabled={isPending} type="datetime-local" {...register('expiresAt')} className="w-full bg-background border border-border  px-3 py-2 text-white focus:outline-none focus:border-brand [&::-webkit-calendar-picker-indicator]:invert" />
           </div>
         </div>
      </div>
