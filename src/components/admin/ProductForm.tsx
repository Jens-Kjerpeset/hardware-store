...
export function ProductForm({ target, categories }: { target: any, categories: any[] }) {
  ...
  } catch (e) { console.error('Failed to parse initial product specs:', e); }
  ...
  } catch (e) { console.error('Failed to parse category fields JSON:', e); }
    ...
    <form onSubmit={handleSubmit(onSubmit as any)} className={`space-y-8 w-full max-w-4xl transition-opacity ${isPending ? 'opacity-50' : 'opacity-100'}`}>
      ...
      <div className="bg-surface border border-border p-6  space-y-6">
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
           <div>
             ...
             <input disabled={isPending} {...register('name')} className="w-full bg-background border border-border  px-3 py-2 text-white focus:outline-none focus:border-brand" />
             {errors.name && <p className="text-red-400 text-xs mt-1">{errors.name.message}</p>}
           </div>
           
           <div>
             <label className="block text-sm font-semibold text-zinc-400 mb-1">Brand</label>
             <input disabled={isPending} {...register('brand')} className="w-full bg-background border border-border  px-3 py-2 text-white focus:outline-none focus:border-brand" />
           </div>

           <div className="md:col-span-2">
             <label className="block text-sm font-semibold text-zinc-400 mb-1">Description</label>
             <textarea disabled={isPending} {...register('description')} rows={4} className="w-full bg-background border border-border  px-3 py-2 text-white focus:outline-none focus:border-brand" />
           </div>

           <div>
             <label className="block text-sm font-semibold text-zinc-400 mb-1">Image URL</label>
             <div className="flex items-center gap-3">
               <div className="w-12 h-12 shrink-0  bg-[#1f1614] border border-border flex items-center justify-center overflow-hidden relative">
                 {currentImgUrl ? (
                   <img 
               ...
               <input disabled={isPending} {...register('imageUrl')} placeholder="/assets/example.jpg" className="w-full bg-background border border-border  px-3 py-2 text-white focus:outline-none focus:border-brand" />
             </div>
           </div>

           <div>
             <label className="block text-sm font-semibold text-zinc-400 mb-1">SKU</label>
             <input disabled={isPending} {...register('sku')} className="w-full bg-background border border-border  px-3 py-2 text-white focus:outline-none focus:border-brand" />
           </div>

            <div>
             <label className="block text-sm font-semibold text-zinc-400 mb-1">Hardware Category</label>
             <select disabled={isPending} {...register('categoryId')} className="w-full bg-background border border-border  px-3 py-2 text-white focus:outline-none focus:border-brand font-medium">
               {categories.map((c) => (
            ...
            <div className="flex items-center gap-3 pt-6">
             <input type="checkbox" disabled={isPending} id="isActive" {...register('isActive')} className="w-4 h-4  bg-background accent-brand" />
      ...
      <div className="bg-surface border border-border p-6  space-y-6">
         <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2">
           ...
           <div className="flex items-center gap-2 bg-background border border-border px-3 py-1.5  text-xs font-bold text-zinc-300">
         ...
         <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
           <div>
             ...
             <input disabled={isPending} type="number" step="0.01" {...register('price')} className="w-full bg-background border border-border  px-3 py-2 text-white focus:outline-none focus:border-brand" />
           </div>
           <div>
             <label className="block text-sm font-semibold text-zinc-400 mb-1">Cost</label>
             <input disabled={isPending} type="number" step="0.01" {...register('cost')} className="w-full bg-background border border-border  px-3 py-2 text-white focus:outline-none focus:border-brand" />
           </div>
           <div>
             <label className="block text-sm font-semibold text-zinc-400 mb-1">Total Stock</label>
             <input disabled={isPending} type="number" {...register('stock')} className="w-full bg-background border border-border  px-3 py-2 text-white focus:outline-none focus:border-brand" />
           </div>
           <div>
             <label className="block text-sm font-semibold text-zinc-400 mb-1">Low Warning</label>
             <input disabled={isPending} type="number" {...register('lowStockThreshold')} className="w-full bg-background border border-border  px-3 py-2 text-white focus:outline-none focus:border-brand" />
           </div>
         </div>
      </div>
               ...
               parsedFieldsJson.map((field) => (
                  <div key={field.name}>
                     ...
                     <input 
                        ...
                        className="w-full bg-surface border border-border  px-3 py-2 text-white focus:outline-none focus:border-brand" 
                     />
                  </div>
               ))
            )}
