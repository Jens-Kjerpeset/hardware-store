  ...
  return (
    <div className="flex flex-col gap-1.5 relative pb-6">
      ...
      <input
        id={name}
        type={type}
        className={`w-full bg-background border px-4 py-2.5 rounded text-white transition-colors focus:outline-none ${
          errorMsg 
      ...
      />
      {errorMsg && (
        <span className="text-[11px] tracking-wide font-semibold text-red-400 bg-red-950/20 border border-red-900/50 rounded px-2 py-0.5 absolute bottom-0 left-0">
          {errorMsg as string}
        </span>
      )}
    </div>
...
export function CheckoutForm() {
  ...
  if (cartItems.length === 0) {
    return (
      <div className="p-16 text-center space-y-6 border border-border bg-surface rounded-lg shadow-xl max-w-2xl mx-auto mt-12">
        ...
        <Link href="/catalog" className="inline-flex mt-4 px-8 py-3.5 bg-zinc-800 hover:bg-zinc-700 text-white font-bold rounded border border-border transition-colors">
          Browse Components
        </Link>
      </div>
    );
  ...
  return (
    <div className="flex flex-col-reverse lg:grid lg:grid-cols-[1fr_400px] gap-8 items-start">
      ...
      <div className="space-y-6 w-full">
        {errorMessage && (
          <div className="p-4 bg-red-950/20 border border-red-900/50 rounded text-red-500 text-sm font-medium">
            {errorMessage}
          </div>
        )}
        ...
        <form id="checkout-form" onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="bg-surface border border-border rounded-lg p-6 lg:p-8 space-y-8">
             ...
             <div className="pt-6 border-t border-border space-y-4">
               ...
               <div className="grid grid-cols-1 gap-3">
                 {[
                   ...
                   <label key={method.id} className={`flex items-center justify-between p-4 border rounded cursor-pointer transition-colors ${activeShippingMethod === method.id ? 'border-brand bg-brand/5' : 'border-border bg-background hover:border-zinc-500'}`}>
             ...
             <div className="space-y-4">
               <div className="grid grid-cols-3 gap-3">
                 {[
                   ...
                   <label key={method.id} className={`flex flex-col items-center justify-center p-3 border rounded cursor-pointer transition-colors text-sm font-medium ${activePaymentMethod === method.id ? 'border-brand bg-brand/5 text-brand' : 'border-border bg-background text-zinc-400 hover:border-zinc-500 hover:text-zinc-200'}`}>
               ...
               <div className="pt-6 border-t border-border grid grid-cols-2 gap-4">
                 <div className="col-span-2">
                   <div className="flex flex-col gap-1.5">
                     ...
                     <input id="cardNumber" type="text" {...register('cardNumber')} placeholder="0000 0000 0000 0000" className="w-full bg-background border border-border px-4 py-2.5 rounded text-white focus:outline-none focus:border-brand" />
                   </div>
                 </div>
                 <div className="flex flex-col gap-1.5">
                   <label className="text-sm font-medium text-zinc-300 ml-1">Expiry Date</label>
                   <input type="text" placeholder="MM/YY" className="w-full bg-background border border-border px-4 py-2.5 rounded text-white focus:outline-none focus:border-brand" />
                 </div>
                 <div className="flex flex-col gap-1.5">
                   <label className="text-sm font-medium text-zinc-300 ml-1">CVC</label>
                   <input type="text" placeholder="123" className="w-full bg-background border border-border px-4 py-2.5 rounded text-white focus:outline-none focus:border-brand" />
                 </div>
               </div>
             )}
          </div>

          <div className="bg-surface border border-border rounded-lg p-6 flex flex-col gap-2 relative pb-8 relative">
             ...
             {errors.acceptTerms && (
               <span className="text-[11px] tracking-wide font-semibold text-red-400 bg-red-950/20 border border-red-900/50 rounded px-2 py-0.5 absolute bottom-3 left-6">
