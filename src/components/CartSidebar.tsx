...
export function CartSidebar({ isOpen, onClose }: CartSidebarProps) {
  ...
  // ----- LOOSE PARTS LOGIC -----
  const looseTotal = cart.reduce((sum, item) => sum + (item.price * item.quantity), 0);
    ...
    <>
      ...
      <div className="fixed inset-y-0 right-0 z-[70] w-full sm:w-[500px] glass bg-dark-bg/95 border-l border-dark-border shadow-2xl flex flex-col animate-in slide-in-from-right duration-300 shadow-2xl">
        ...
        <div className="flex items-center justify-between p-6 border-b border-dark-border">
          ...
          <button onClick={onClose} className="p-2 text-gray-400 hover:text-white bg-dark-surface transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {!mode && (
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center text-gray-400">
             ...
             <button onClick={() => { onClose(); router.push('/'); }} className="mt-4 text-brand-500 hover:text-brand-400 font-medium">Return to Home</button>
          </div>
        )}
          ...
          <>
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {cart.length === 0 ? (
                ...
                cart.map(item => (
                  <div key={item.id} className="flex items-start gap-4 p-4 border border-dark-border bg-dark-surface/30">
                     ...
                     <div className="flex flex-col items-end gap-2">
                       <div className="flex items-center gap-2 bg-dark-bg border border-dark-border p-1">
                          <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="w-6 h-6 flex items-center justify-center text-gray-400 hover:text-white bg-white/5">-</button>
                          <span className="text-xs font-mono w-4 text-center">{item.quantity}</span>
                          <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="w-6 h-6 flex items-center justify-center text-gray-400 hover:text-white bg-white/5">+</button>
                       </div>
                       <button onClick={() => removeFromCart(item.id)} className="text-xs text-red-500 hover:text-red-400 mt-1">Remove</button>
                     </div>
                  </div>
                ))
              )}
            ...
            <div className="p-6 border-t border-dark-border bg-dark-bg">
               ...
               <button onClick={clearCart} className="w-full mt-3 text-sm text-gray-500 hover:text-red-400 py-2 transition-colors">
                 Empty Cart
               </button>
            </div>
          </>
